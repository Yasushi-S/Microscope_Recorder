...
# CAP_MSMFはこのカメラだとオープン＋プロパティ設定（解像度/FPS）の1回ごとに
# 約13秒かかり、起動完了まで合計50秒以上を要する（実機計測済み・解像度に関わらず一定）。
# その間「何も映らない」状態を避けるため、オープン自体は速いCAP_DSHOWで低解像度の
# つなぎ映像を即座に表示し、MSMFの初期化が終わったら自動的に切り替える。
BRIDGE_CAPTURE_SIZE = (1280, 720)    # つなぎ映像用解像度（CAP_DSHOWで実測10fps程度）
...
# 検体種別ごとの保存先サブフォルダ（帯下(生食)/帯下(KOH)は「帯下」にまとめる）
SPECIMEN_FOLDER = {
    "帯下(生食)": "帯下",
    "帯下(KOH)": "帯下",
    "精液": "精液",
...
def save_dir_for_specimen(specimen: str) -> str:
    return os.path.join(SAVE_DIR, SPECIMEN_FOLDER.get(specimen, "その他"))
...
class MicroscopeApp:
    def __init__(self, root: tk.Tk):
        ...
        # iconbitmap()はタイトルバーの小アイコンしか反映されずタスクバーの大アイコンが
        # 既定のまま（空表示）になることがあるため、iconphoto()で両方に反映させる。
        ...
        for folder in set(SPECIMEN_FOLDER.values()):
    ...
    def _build_ui(self):
        ...
        # 保存フォルダ表示
        tk.Label(right, text=f"保存先: {SAVE_DIR}\\[帯下|精液|その他]",
                 font=("Meiryo UI", 8),
    ...
    def _refresh_file_list(self):
        ...
        for folder in set(SPECIMEN_FOLDER.values()):
            ...
            try:
                ...
                continue
            for f in files:
                entries.append((os.path.join(folder_path, f), folder, f))
        entries.sort(key=lambda e: os.path.getmtime(e[0]), reverse=True)
        self.file_listbox.delete(0, tk.END)
        for _, folder, f in entries:
            icon = "🎬" if f.endswith(".mp4") else "📷"
            self.file_listbox.insert(tk.END, f"{icon} [{folder}] {f}")
    ...
    def _update_fname_preview(self):
        ...
        folder = SPECIMEN_FOLDER.get(sp, "その他")
        self.fname_var.set(f"例: {folder}\\{base}.mp4 / {folder}\\{base}.jpg")
    ...
    def _start_preview(self):
        # CAP_MSMFのオープン＋設定（_main_preview_loop）は完了まで50秒以上かかるため、
        # その間は起動が速いCAP_DSHOWのつなぎ映像（_bridge_preview_loop）を並行して
        # 表示し、MSMFの準備が整った時点で自動的に切り替える。
        self.preview_active = True
        self._bridge_active = True
        self.bridge_thread = threading.Thread(target=self._bridge_preview_loop, daemon=True)
        self.bridge_thread.start()
        self.preview_thread = threading.Thread(target=self._main_preview_loop, daemon=True)
        self.preview_thread.start()

    def _bridge_preview_loop(self):
        # つなぎ映像：CAP_DSHOWはMJPEGデコードがボトルネックで5～10fps程度しか出ないが
        # （実機検証済み）、オープン自体は数秒で終わるため起動直後の表示用に使う。
        # MSMFのカメラと同時に開いても競合しないことは実機検証済み。
        cap = cv2.VideoCapture(CAMERA_INDEX, cv2.CAP_DSHOW)
        if not cap.isOpened():
            cap.release()
            return
        cap.set(cv2.CAP_PROP_FOURCC, cv2.VideoWriter_fourcc(*"MJPG"))
        cap.set(cv2.CAP_PROP_FRAME_WIDTH, BRIDGE_CAPTURE_SIZE[0])
        cap.set(cv2.CAP_PROP_FRAME_HEIGHT, BRIDGE_CAPTURE_SIZE[1])
        while self.preview_active and self._bridge_active:
            ret, frame = cap.read()
            if not ret:
                time.sleep(0.05)
                continue
    ...
    def _main_preview_loop(self):
        ...
        if not cap.isOpened():
            ...
            # MSMFが使えない場合でも、つなぎ映像（CAP_DSHOW）はそのまま表示を続ける。
            return
        ...
        # 本番映像の準備ができたので、つなぎ映像を停止してから表示を切り替える
        # （スレッドの終了を待ってからでないと、つなぎ映像の描画が後から上書きされる恐れがある）
        self._bridge_active = False
    ...
    def _capture_photo(self):
        ...
        filepath = generate_filename(patient_id, self.specimen_var.get(),
                                      save_dir_for_specimen(self.specimen_var.get()), ext="jpg")
    ...
    def _start_record(self):
        ...
        filepath = generate_filename(patient_id, self.specimen_var.get(),
                                      save_dir_for_specimen(self.specimen_var.get()))
        cmd = [
            "ffmpeg", "-y",
...
if __name__ == "__main__":
    # pythonw.exeという共通の実行ファイル名のままだと、Windows 11のタスクバーが
    # アイコンをアプリ単位で正しく認識せず空アイコン表示になることがあるため、
    # 専用のAppUserModelIDを設定してこのアプリを独立した存在として識別させる。
    try:
        ctypes.windll.shell32.SetCurrentProcessExplicitAppUserModelID("Clinic.MicroscopeRecorder")
    except Exception:
        pass
